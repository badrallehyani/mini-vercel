...
router.get(/(.*)/, async (req: Request, res: Response, next: Function) => {
    ...
    let filePath = req.path.endsWith('/') ? `${req.path}index.html` : req.path;
    ...
    try {
        const objectData = await getObject(fullPath, 'outputs');
...
router.get('/upload-github', async (req: Request, res: Response) => {
    ...
    if (type === PROJECTS_TYPES.NPM) {
        ...
        await buildAndRunDockerImage(repoURL, projectID);
        res.send('Building Docker image. Check logs for progress.');
        return;
    }

    if (type === PROJECTS_TYPES.VANILLA) {
        try {
